...
bool print_winner(void)
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        //Verify later if need to add 0.5
...
int find_min(void)
{
    ...
    {
        if (candidates[p].votes < min_vote && candidates[p].eliminated == false)
        {
...
bool is_tie(int min)
{
    //Check if there are any higher votes
    for (int y = 0; y < candidate_count; y++)
    {
        if (candidates[y].votes > min && candidates[y].eliminated == false)
        {
            return false;
        }
    }
